  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Reset Form
        setNewTask({ title: '', description: '', status: 'TODO', priority: 'MEDIUM', team: '', assignee: '', due_date: '' });
        fetchAllData(); // Refresh board
      }
    } catch (err) {
              ...
              {tasks.filter(t => t.status === status).map(task => (
                <div key={task.id} className="group relative bg-[#18181b] border border-zinc-800 rounded-xl p-4 hover:border-zinc-600 transition-all shadow-sm">
                  ...
                  <div className="absolute top-3 right-3 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-[#18181b] p-1 rounded-lg border border-zinc-700">
                    {task.status !== 'TODO' && <button onClick={() => handleMove(task, 'left')} className="p-1 hover:bg-zinc-700 rounded text-zinc-400"><ChevronLeft size={14}/></button>}
                    {task.status !== 'DONE' && <button onClick={() => handleMove(task, 'right')} className="p-1 hover:bg-zinc-700 rounded text-zinc-400"><ChevronRight size={14}/></button>}
                    <button onClick={() => handleDelete(task.id)} className="p-1 hover:bg-red-900/30 text-red-400 rounded"><Trash2 size={14}/></button>
                  </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#18181b] border border-zinc-700 w-full max-w-lg rounded-2xl p-6 relative shadow-2xl">
            <button onClick={() => setIsModalOpen(false)} className="absolute top-4 right-4 text-zinc-400 hover:text-white"><X size={20} /></button>
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <textarea 
                  placeholder="Details..."
                  className="w-full bg-[#09090b] border border-zinc-700 rounded-lg p-3 text-white text-sm outline-none focus:border-[#6366F1] h-24 resize-none"
