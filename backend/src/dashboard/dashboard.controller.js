...
const createQuickTask = async (req, res) => {
  try {
    const { title, priority, assignee, description, userId, userName } = req.body;
    await db.query(
      'INSERT INTO tasks (title, description, priority, status, assignee) VALUES (?, ?, ?, "TODO", ?)',
      [title, description || '', priority || 'MEDIUM', assignee || 'Unassigned']
    );
